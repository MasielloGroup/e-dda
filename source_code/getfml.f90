    SUBROUTINE GETFML(AKR,AK3,AKSR,BETADF,PHIDF,THETADF,DX,X0,CALPHA,  &
                        ...
                        h_bar,h_bar2,velocity,e_charge,                  &
...
! Arguments:
      ...
      REAL(WP) ::          &
         ...
         YLR(3), &
         ZLR(3), &
         RM(3,3)   !AEFFA and after by NWB 3/8/12
         !XLR,YLR, ZLR added by NWB 12/22/14 after SMC 15.5.13
...
!                       MXN3,1,h_bar,h_bar2) !All parameters h_bar and after added by NWB 7/11/12
            CALL EVALQ(CXADIA,CXAOFF,NAT03,CXE,CXPOL,CABS,CEXT,CPHA,1,MXN3,h_bar,h_bar2,MXRAD,AEFFA, &
                 NAT0,c,MXWAV,WAVEA)

            !QABS(JO)=CABS/PIA2
            QABS(JO)=CABS !NEW CODE NWB 12/17/14
            QEXT(JO)=CEXT !NEW CODE NWB 03/08/12
            QPHA(JO)=CPHA/PIA2
...
!*** second call to timing routine
         ...
         QABS(1)=CABS !NEW CODE NWB 12/17/14
         QEXT(1)=CEXT !NEW CODE NWB 03/08/12
         QPHA(1)=CPHA/PIA2
...
!*** second call to timing routine
         ...
         QABS(2)=CABS !NEW CODE NWB 12/17/14
         QEXT(2)=CEXT !NEW CODE NWB 03/08/12
         QPHA(2)=CPHA/PIA2
